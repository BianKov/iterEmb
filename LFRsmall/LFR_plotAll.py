...
#    fileHandler=open(os.getcwd()+"/LFR_"+dirNameDict[mu]+"/TREXPICresults.txt", "r")
...
#    AMI_Infomap[mu] = float(listOfLines[28])
        ...
        fileHandler=open(os.getcwd()+"/LFR_"+dirNameDict[mu]+"/graph1/node2vecResults.txt", "r")
        listOfLines=fileHandler.readlines()
        fileHandler.close()
        AMI_node2vec[mu] = float(listOfLines[3])
    
        fileHandler=open(os.getcwd()+"/LFR_"+dirNameDict[mu]+"/graph1/TREXPICresults.txt", "r")
        listOfLines=fileHandler.readlines()
        fileHandler.close()
        AMI_TREXPIC[mu] = float(listOfLines[3])

        fileHandler=open(os.getcwd()+"/LFR_"+dirNameDict[mu]+"/graph1/LouvainResults.txt", "r")
        listOfLines=fileHandler.readlines()
        fileHandler.close()
        AMI_Louvain[mu] = float(listOfLines[3])
    
        fileHandler=open(os.getcwd()+"/LFR_"+dirNameDict[mu]+"/graph1/InfomapResults.txt", "r")
        listOfLines=fileHandler.readlines()
        fileHandler.close()
        AMI_Infomap[mu] = float(listOfLines[3])

        fileHandler=open(os.getcwd()+"/LFR_"+dirNameDict[mu]+"/graph1/alabpropResults.txt", "r")
        listOfLines=fileHandler.readlines()
        fileHandler.close()
        AMI_alabprop[mu] = float(listOfLines[3])


#    fileHandler=open(os.getcwd()+"/LFR_"+dirNameDict[mu]+"/node2vecResults_Birch.txt", "r")
#    listOfLines=fileHandler.readlines()
#    fileHandler.close()
#    AMI_node2vec_Birch[mu] = float(listOfLines[5])
    
#    fileHandler=open(os.getcwd()+"/LFR_"+dirNameDict[mu]+"/node2vecResults_firstEmb_Birch.txt", "r")
#    listOfLines=fileHandler.readlines()
#    fileHandler.close()
...
plt.title('LFR networks, N=1000, avDeg=20, maxDeg=50, degExp=2, commExp=3, minc=10, maxc=100')
fig.savefig(os.getcwd()+'/smallLFRresults_graph1.png',bbox_inches="tight",dpi=300) #dpi=300 kell minimum
plt.close(fig)
