...
graphID = 0 #https://link.aps.org/accepted/10.1103/PhysRevE.103.022316 : 20 networks at each setting
...
for mu in muList:
    ...
    #set the q multiplying factor that will be used for mapping the angular distances to link weights
    degreeList = [G_original.degree(nbunch=nodeName,weight=None) for nodeName in G_original.nodes()]
    try:
    ...
    print('node2vec - 1')
    G_current=em.node2vec(G_previous,qFactor)
    ...
    while relChangeInAvgWeight>wDiffOfStop and numOfDoneEmbeddings<maxNumOfEmbeddings:
        ...
        print('node2vec - '+str(numOfDoneEmbeddings+1))
        G_current=em.node2vec(G_previous,qFactor)
    ...
    weightList.sort(reverse=True) #decreasing order of the link weights (i.e., connection probabilities)
    ...
    ReportFileHandler = open(os.getcwd() + "/" + directoryName + "/node2vecResults_readable.txt", 'w')
    ...
    ReportFileHandler = open(os.getcwd() + "/" + directoryName + "/node2vecResults.txt", 'w')
