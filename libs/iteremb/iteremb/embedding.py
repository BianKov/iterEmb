...
from scipy import sparse #needed for TREXPIC, Isomap (singular value decomposition) and Laplacian Eigenmaps (eigendecomposition)
...
def loadGraph(path,skipRows=0,delimiter="\t"):
    edgeList = [] #initialize the list of the (source node identifier,target node identifier,link weight) edge 3-tuples
    ...
    while True:
        ...
        if listOfWords[1][-1]=="\n": #the second column is the last in the currently loaded line
            ...
            if listOfWords[2][-1]=="\n": #the third column is the last in the currently loaded line
                linkWeight = float(listOfWords[2][:-1]) #convert the string from the third column without "\n" to a float
        ...
        if sourceNodeID != targetNodeID: #the self-loops are disregarded
            if linkWeight == None: #set all the link weights to 1.0 (the embedding functions will assume that the links have a weight attribute)
    ...
    G_total = nx.Graph()
    G_total.add_weighted_edges_from(edgeList,weight='weight') #multi-edges are automatically converted to single edges -> when loading a weighted edge list, please include all the links with only the one required weight, otherwise only the lastly appearing weight value will be stored
...
def TREXPIC(G,d,q=None,K=-1):
    ...
    if N < d+1:
        print('\n\nERROR: The number d of embedding dimensions in the function embedding.TREXPIC can not be larger than the number of nodes-1, i.e. '+str(N-1)+'.\n\n')
    ...
    L = np.ones((N,N)) #L[i][j] is the expected Lorentz product calculated from the length of the shortest path from the ith node to the jth node, where the nodes are ordered according to listOfNodes
    for rowID in range(1,N):
        for colID in range(rowID): #iterate over the node pairs
            ...
            try:
                expDistance = math.exp(-q/shortestPathLengthsDict[listOfNodes[rowID]][listOfNodes[colID]])
            except KeyError: #there is no path between the given two nodes -> set the distance to the possible highest value (SPL=infinity -> exp(-q/SPL)=exp(0)=1)
                expDistance = 1.0
    ...
    if d == N-1:
        ...
        U,S,VT = sparse.linalg.svds(L,d+1,which='LM',solver='arpack') #the singular values are ordered from the smallest to the largest in S (increasing order)
    ...
    if numOfPositiveSingularValues<d+1:
        print('\n\nERROR: The number d of embedding dimensions in the function embedding.TREXPIC can not be larger than the number of positive singular values of the Lorentz matrix-1, i.e. '+str(numOfPositiveSingularValues-1)+'.\n\n')
    ...
    for nodeName in listOfNodes:
        ...
        directionArray = np.multiply(Uarray[1:],Ssqrt) #the jth element is the jth coordinate of the node named nodeName in the reduced space
                    #we could also use: directionArray = np.multiply(Varray[1:],Ssqrt)
        originalNorm = np.linalg.norm(directionArray)
        Coord[nodeName] = r_native*directionArray/originalNorm #the Cartesian coordinates of the node named nodeName in the native representation of the d-dimensional hyperbolic space
    ...
    for (i,j) in G.edges(): #(i,j)=a tuple, which denotes the edge between node u and v
        # assign a weight to the i-j edge:
        w = 1.0-(np.inner(Coord[i],Coord[j]) / (np.linalg.norm(Coord[i])*np.linalg.norm(Coord[j]))) #weight=cosine distance=1-cos(dTheta)
...
def expISO(G,d,q=None):
    N = len(G) #the number of nodes in graph G
    if N < d:
        print('\n\nERROR: The number d of embedding dimensions in the function embedding.ISO can not be larger than the number of nodes, i.e. '+str(N)+'.\n\n')
    ...
    listOfNodes = list(G.nodes())
    D = np.zeros((N,N)) #D[i][j] is the expected Euclidean distance calculated from the length of the shortest path from the ith node to the jth node, where the nodes are ordered according to listOfNodes
    for rowID in range(1,N):
        for colID in range(rowID): #iterate over the node pairs
    ...
    #dimension reduction
    if d == N:
        U,S,VT=np.linalg.svd(IP) #find all the N number of singular values and the corresponding singular vectors (with decreasing order of the singular values in S)
             #note that for real matrices the conjugate transpose is just the transpose, i.e. V^H=V^T
    else: #d<N: only the first d largest singular values (and the corresponding singular vectors) are retained
        U,S,VT = sparse.linalg.svds(IP,d,which='LM',solver='arpack') #the singular values are ordered from the smallest to the largest in S (increasing order)
        #reverse the order of the singular values to obtain a decreasing order, and reverse the order of singular vectors accordingly:
        S = S[::-1]
        U = U[:,::-1]
        VT = VT[::-1]
    ...
    if numOfPositiveSingularValues<d:
        print('\n\nERROR: The number d of embedding dimensions in the function embedding.expISO can not be larger than the number of positive singular values of the inner product matrix, i.e. '+str(numOfPositiveSingularValues)+'.\n\n')
    Ssqrt = np.sqrt(S)
    ...
    Coord = {} #initialize a dictionary that assigns to the node names NumPy arrays of d elements containing the Cartesian coordinates of the network nodes in the d-dimensional Euclidean space
    nodeIndex = 0
    for nodeName in listOfNodes:   
        #calculate the position of the given node
        Uarray = U[nodeIndex,:]
        Coord[nodeName] = np.multiply(Uarray,Ssqrt) #the jth element is the jth Cartesian coordinate of the node named nodeName in the reduced space
...
def ISO(G,d):
    ...
    #create the matrix of expected Euclidean distances
    listOfNodes = list(G.nodes())
    D = np.array([[shortestPathLengthsDict[s][t] for t in listOfNodes] for s in listOfNodes]) #D[i][j] is the expected Euclidean distance given by the length of the shortest path from the ith node to the jth node, where the nodes are ordered according to listOfNodes
    ...
    numOfPositiveSingularValues = np.sum(S > 0)
    if numOfPositiveSingularValues<d:
        print('\n\nERROR: The number d of embedding dimensions in the function embedding.ISO can not be larger than the number of positive singular values of the inner product matrix, i.e. '+str(numOfPositiveSingularValues)+'.\n\n')
    Ssqrt = np.sqrt(S)
...
def LE(G,d,scalingFactor=None):
    N = len(G) #the number of nodes in graph G
    if N < d+1:
        print('\n\nERROR: The number d of embedding dimensions in the function embedding.LE can not be larger than the number of nodes-1, i.e. '+str(N-1)+'.\n\n')
    ...
    if len(set(listOfWeights))==1: #all the link weights are equal, i.e. the network is practically unweighted -> simple-minded Laplacian Eigenmaps
        G_forEmbedding = nx.Graph()
        G_forEmbedding.add_nodes_from(G.nodes) #keep the node order of the original graph
        for (i,j) in G.edges(): #(i,j)=a tuple, which denotes the edge between node u and v
            # assign a weight to the i-j edge:
            w = float(G[i][j]['weight']) #the function that performs the eigendecomposition needs matrixes of floats
        ...
        if scalingFactor==None: #use the default setting
            scalingFactor = math.pow(np.mean(listOfWeights),2) #default: the square of the average of all the link weights, as in https://www.nature.com/articles/s41467-017-01825-5
    ...
    listOfNodes = list(G.nodes())
    adjMatrix = nx.to_scipy_sparse_matrix(G_forEmbedding,nodelist=listOfNodes,weight='weight',format="csr") #adjacency matrix as a SciPy sparse matrix
    ...
    degreeMatrix = sparse.spdiags(adjMatrix.sum(axis=1).reshape(1,N), 0, numOfCols, numOfRows, format="csr") #degrees in a SciPy sparse matrix
    ...
    eigenvalues, eigenvectors = sparse.linalg.eigsh(graphLaplacian,k=d+1,M=degreeMatrix,which='SM',ncv=min(N,max(2*(2*(d+1)+1),2*20))) #find k eigenvalues and eigenvectors of the real symmetric square matrix graphLaplacian
    ...
    eigenvectors = eigenvectors[:,1:d+1] #omit the eigenvector of 0 eigenvalue and keep only d number of the following eigenvectors
...
def expNode2vec(G,q,d=64,walkLength=10,numOfWalks=80,windowLength=10,batchWalks=10000,pForEmbedding=1.0,qForEmbedding=1.0):
    #create the embedding
    listOfNodes = list(G.nodes())
    adjMatrix = nx.adjacency_matrix(G,nodelist=listOfNodes,weight='weight') #adjacency matrix as a SciPy Compressed Sparse Row (csr) matrix
    ...
    for (i,j) in G.edges(): #(i,j)=a tuple, which denotes the edge between node u and v
        # assign a weight to the i-j edge:
        w_notExp = (np.inner(Coord[i],Coord[j]) / (np.linalg.norm(Coord[i])*np.linalg.norm(Coord[j])))-1 #w_notExp=cos(angular distance)-1: nonpositive, proximity-like weights
        if w_notExp<-2: #a numerical error has occurred
            w_notExp = -2.0
        if w_notExp>0: #a numerical error has occurred
            w_notExp = 0.0
        w = math.exp(q*w_notExp) #link weight=exponentialized form of a measure composed from cosine similarity
        G_embWeighted.add_edge(i,j,weight=w)
...
def node2vec(G,d=64,walkLength=10,numOfWalks=80,windowLength=10,batchWalks=10000,pForEmbedding=1.0,qForEmbedding=1.0):
    #create the embedding
    listOfNodes = list(G.nodes())
    adjMatrix = nx.adjacency_matrix(G,nodelist=listOfNodes,weight='weight') #adjacency matrix as a SciPy Compressed Sparse Row (csr) matrix
                #in newer versions of NetworkX: adjMatrix = sparse.csr_matrix(nx.adjacency_matrix(G,nodelist=listOfNodes,weight='weight'))
    ...
    for (i,j) in G.edges(): #(i,j)=a tuple, which denotes the edge between node u and v
        # assign a weight to the i-j edge:
        w = (np.inner(Coord[i],Coord[j]) / (np.linalg.norm(Coord[i])*np.linalg.norm(Coord[j])))+1 #link weight=cos(angular distance)+1: nonnegative, proximity-like weights
        if w<0: #a numerical error has occurred
            w = 0.0
        if w>2: #a numerical error has occurred
            w = 2.0
