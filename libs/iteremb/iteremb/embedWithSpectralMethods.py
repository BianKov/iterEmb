...
d = em.choosingDim(G_original,os.getcwd()+'/'+directoryName+'/automaticDimensionSelectionForSpectralMethods.png')
...
print('LE - 1')
[G_current,Coord_current]=em.LE(G_original,d) #embed the network with Laplacian Eigenmaps
...
groupDict_detected = cD.commDetWithWeightThresholding(G_current,'small') #the link weights mean distances -> keep the links having a weight below a threshold
#calculate the modularity
...
while relChangeInAvgWeight>wDiffOfStop and numOfDoneEmbeddings<maxNumOfEmbeddings:
    ...
    [G_current,Coord_current]=em.LE(G_previous,d) #embed the network with Laplacian Eigenmaps
...
np.savetxt(os.getcwd() + "/" + directoryName + "/LE_angSeparation.txt", separationMeasureList)
np.savetxt(os.getcwd() + "/" + directoryName + "/LE_angSepNumerator.txt", numeratorList)
np.savetxt(os.getcwd() + "/" + directoryName + "/LE_angSepDenominator.txt", denominatorList)

#save the community detection performances in the case of the simple weight thresholding
ReportFileHandler = open(os.getcwd() + "/" + directoryName + "/LE_commDetWithThresholding.txt", 'w')
#properties of the examined network
ReportFileHandler.write('numOfNodes = '+str(numOfNodes)+'\n')
ReportFileHandler.write('numOfEdges = '+str(numOfEdges)+'\n')
ReportFileHandler.write('numOfComms_real = '+str(numOfComms_real)+'\n')
