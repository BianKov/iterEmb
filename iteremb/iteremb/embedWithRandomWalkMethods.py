...
[AMI_Louvain, ARI_Louvain, elemCentSimilarity_Louvain] = cD.calculateSimilarityMeasures(
    groupDict_real, groupDict_detected
)
...
degreeList = [
    G_original.degree(nbunch=nodeName, weight=None) for nodeName in G_original.nodes()
]  # Consider here the number of connections, not the sum of weights! (This does not change during the iterations, only the link weights.)
degreeFrequencyDict = Counter(
    degreeList
)  # key=degree, value=number of nodes having the given degree
highestDegreeFrequency = max(degreeFrequencyDict.values())
mostFrequentDegrees = [
    deg
    for deg in degreeFrequencyDict.keys()
    if degreeFrequencyDict[deg] == highestDegreeFrequency
]
degreeMode = min(
    mostFrequentDegrees
)  # the smallest degree value from those that occurrs the most frequently
...
[G_current, Coord_current] = em.expNode2vec(
    G_original, qFactor, d
)  # embed the network with node2vec and weight the link according to an exponential formula
...
for node1ID in range(1, numOfNodes):
    ...
    for node2ID in range(node1ID):
        ...
        argOfAcos = np.inner(Coord_current[nodeName1], Coord_current[nodeName2]) / (
...
groupDict_detected = cD.commDetWithWeightThresholding(
    G_current, "large"
)  # the link weights mean proximities -> keep the links having a weight above a threshold
# calculate the modularity
...
np.savetxt(
    os.getcwd() + "/" + directoryName + "/expNode2vec_angSeparation.txt",
    separationMeasureList,
)
np.savetxt(
    os.getcwd() + "/" + directoryName + "/expNode2vec_angSepNumerator.txt",
    numeratorList,
)
np.savetxt(
    os.getcwd() + "/" + directoryName + "/expNode2vec_angSepDenominator.txt",
    denominatorList,
)

# save the community detection performances in the case of the simple weight thresholding
ReportFileHandler = open(
    os.getcwd() + "/" + directoryName + "/expNode2vec_commDetWithThresholding.txt", "w"
)
# properties of the examined network
ReportFileHandler.write("numOfNodes = " + str(numOfNodes) + "\n")
ReportFileHandler.write("numOfEdges = " + str(numOfEdges) + "\n")
...
ReportFileHandler.write(
    "largestRelCommSize_wThreshold_1 = " + str(largestRelCommSize_wThreshold_1) + "\n"
)
...
ReportFileHandler.write(
    "elemCentSimilarity_wThreshold_1 = " + str(elemCentSimilarity_wThreshold_1) + "\n"
)
...
ReportFileHandler = open(
    os.getcwd() + "/" + directoryName + "/expNode2vec_Louvain.txt", "w"
)
# properties of the examined network
ReportFileHandler.write("numOfNodes = " + str(numOfNodes) + "\n")
ReportFileHandler.write("numOfEdges = " + str(numOfEdges) + "\n")
...
ReportFileHandler.write(
    "largestRelCommSize_Louvain_conv = " + str(largestRelCommSize_Louvain_conv) + "\n"
)
...
ReportFileHandler.write(
    "elemCentSimilarity_Louvain_conv = " + str(elemCentSimilarity_Louvain_conv) + "\n"
)
# properties of embedding
ReportFileHandler.write(
    "number of embeddings at convergence = " + str(numOfEmbsAtConv) + "\n"
...
ReportFileHandler = open(
    os.getcwd() + "/" + directoryName + "/expNode2vec_kMeans.txt", "w"
)
# properties of the examined network
ReportFileHandler.write("numOfNodes = " + str(numOfNodes) + "\n")
ReportFileHandler.write("numOfEdges = " + str(numOfEdges) + "\n")
...
ReportFileHandler.write(
    "largestRelCommSize_kMeans_conv = " + str(largestRelCommSize_kMeans_conv) + "\n"
)
...
ReportFileHandler.write(
    "elemCentSimilarity_kMeans_conv = " + str(elemCentSimilarity_kMeans_conv) + "\n"
)
# properties of embedding
ReportFileHandler.write(
    "number of embeddings at convergence = " + str(numOfEmbsAtConv) + "\n"
