...
def commDetWithWeightThresholding(G,whichWeightToKeep,minRemovalRate=0.05,maxRemovalRate=0.95):
    ...
    partition = {} #initialize the dictionary with the obtained partition (key=a node's name,value=its community); the communities are numbered from 0 with integers
    groupID = 0
...
def Louvain(G):
    ...
    partition = cmL.best_partition(G) #dictionary with the obtained partition (key=a node's name,value=its community); the communities are numbered from 0 with integers
    return partition
...
def Infomap(G):
    ...
    nodeNameNodeIDdict = {nodeList[i]:i for i in range(len(nodeList))} #key=node name, value=node ID as an unsigned int
    ...
    for (i,j) in G.edges():
        network.addLink(nodeNameNodeIDdict[i],nodeNameNodeIDdict[j],G.get_edge_data(i,j)['weight']) #expected argument types: unsigned int,unsigned int,double
    infomInst.run() #run the Infomap search algorithm to find optimal modules
    partition={} #dictionary with the obtained partition (key=a node's name, value=its community); the communities are numbered from 0 with integers
...
#G is the examined NetworkX graph
def asynLabProp(G):
    comms=alabprop(G,weight='weight') #iterable of communities given as sets of node names
    partition={} #dictionary with the obtained partition (key=a node's name,value=its community); the communities are numbered from 0 with integers
    groupID=0
    ...
    for groupID in range(numOfOrigGroups):
        ...
        S = G.subgraph(nodeNamesInGroup) #the subgraph spanned by the nodes of the group having the identifier groupID
        if nx.is_connected(S)==False: #the subgraph consists of multiple disconnected parts -> the corresponding group has to be splitted
            compList = [comp for comp in nx.connected_components(S)] #compList[i] is the set of node names in the ith component of the group with the identifier groupID
            numOfNewGroups = len(compList)-1 #the number of new groups that have to be created (one of the subgroups will keep the original groupID)
...
def kMeansClustering(CoordDict,numOfGroups):
    ...
    commStruct_kMeans = clustering.labels_ #an array of integers where the ith element describes the label of the community to which the ith node in listOfNodes belongs
    partition = {} #initialize the dictionary with the obtained partition (key=a node's name,value=its community); the communities are numbered from 0 with integers
    nodeID = 0
    for nodeName in listOfNodes:
...
def calculateSimilarityMeasures(realComms,detectedComms):
    ...
    listOfRelevantNodes = [nodeName for nodeName in realComms.keys() if commSizeDict[realComms[nodeName]]>1]
        
    #calculate AMI and ARI
    realCommList_relevant = [realComms[nodeName] for nodeName in listOfRelevantNodes] #list of the real community labels
    detectedCommList_relevant = [detectedComms[nodeName] for nodeName in listOfRelevantNodes] #list of the detected community labels
    AMI = adjusted_mutual_info_score(realCommList_relevant,detectedCommList_relevant,average_method='max') #the 'max' method results in the smallest AMI value from all the possible normalizations -> calculate the possible worst AMI
    ARI = adjusted_rand_score(realCommList_relevant,detectedCommList_relevant)

    #calculate the element-centric similarity   
    realCommDict_relevant = {nodeName:[realComms[nodeName]] for nodeName in listOfRelevantNodes} #Here non-overlapping communities are assumed!
    detectedCommDict_relevant = {nodeName:[detectedComms[nodeName]] for nodeName in listOfRelevantNodes} #Here non-overlapping communities are assumed!
...
def calculateModularity(G,groupDict):
    Q = modularity(G, [{node for node in groupDict.keys() if groupDict[node]==group} for group in set(groupDict.values())], weight=None)
